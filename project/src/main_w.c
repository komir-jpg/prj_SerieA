...
void create_menu(WINDOW *menu_w) {
    ...
    size_t vec_size = ARR_SIZE(choices);

    items = (ITEM **)calloc(vec_size, sizeof(ITEM *));
    for (int i = 0; i < vec_size; i++) {
        items[i] = new_item(main_menu_config[i].label, " ");
        set_item_usrptr(item[i], (void *)main_menu_config[i].action);
    }
    items[vec_size] = NULL;
...
void print_standings(WINDOW *w, Standings *s) {
    ...
    for (uint i = 0; i < s->num_teams; i++) {
        mwprintw(w, start_y + i, 1,
                 "%-3d %-20.19s %5d %5d %5d %5d %5d %5d %5d %5d", s->ts[i].pos,
                 s->ts[i]->t->name, s->ts[i].played, s->ts[i].win,
                 s->ts[i].draw, s->ts[i].loss, s->ts[i].g_scored,
                 s->ts[i].g_conceded, s->ts[i].g_diff);
    }
}

void show_staindings(PANEL *p, Standigs *s) {
    /*
    ...
    WINDOW *standings_w = create_window(h_wstandings, w_wstandings);
